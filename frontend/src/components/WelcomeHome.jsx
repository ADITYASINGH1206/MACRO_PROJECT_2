    ...
    <div className={`min-h-screen ${styles.surface} ${styles.textPrimary} font-sans selection:bg-primary/20 overflow-hidden relative`}>
      ...
      <nav className="fixed top-0 w-full z-50 backdrop-blur-xl border-b border-white/10 px-8 pt-8 pb-6 flex items-center justify-between" style={{ backgroundColor: isDarkMode ? 'rgba(10,15,28,0.85)' : 'rgba(248,249,252,0.85)' }}>
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-lg font-black tracking-[-0.04em] uppercase">Veri<span className={styles.accentPrimary}>Face</span></h1>
        </div>
        <div className="flex gap-8">
          <button onClick={onNavigateToLogin} className="text-[11px] font-black uppercase tracking-[0.2em] opacity-60 hover:opacity-100 transition-all duration-200">Login</button>
          <button onClick={onNavigateToRegister} className={`px-6 py-2.5 rounded-xl border border-white/10 ${styles.accentPrimary} text-[11px] font-black uppercase tracking-[0.2em] hover:bg-white/5 hover:border-white/20 transition-all duration-200`}>Register</button>
        </div>
      </nav>
      ...
      <main className="relative pt-48 pb-24 px-8 max-w-[1200px] mx-auto flex flex-col items-center text-center">
        ...
        <div className="space-y-12 animate-fade-up">
          <div className="inline-flex items-center gap-3 px-5 py-2 rounded-full bg-white/5 border border-white/10 mb-8">
            ...
            <span className="text-[10px] font-black tracking-[0.3em] uppercase opacity-70">Identity-First Attendance Management</span>
          </div>
          ...
          <p className={`${styles.textSecondary} text-lg font-light max-w-3xl mx-auto leading-relaxed`}>
            VeriFace is a sophisticated biometric attendance system designed to enhance academic integrity with advanced technology. Our platform utilizes state-of-the-art facial recognition and multi-portal synchronization to provide precise, real-time tracking of student attendance, eliminating proxies and automating management workflows for both faculty and students.
          </p>
        ...
        <section className="mt-48 grid grid-cols-1 md:grid-cols-3 gap-12 w-full text-left stagger">
          {[
            ...
            },
            {
              ...
              desc: 'Real-time attendance analytics delivering actionable insights to faculty and students.',
              color: 'text-[#c0c1ff]'
            },
            {
              icon: 'security',
        ...
        <section className="mt-64 w-full">
          <div className="text-center mb-24">
            <h2 className="text-5xl font-black tracking-tighter mb-4">Meet the <span className={styles.accentPrimary}>Designers & ML Team</span></h2>
            <p className={`${styles.textSecondary} text-sm uppercase tracking-[0.3em] font-bold`}>The design & logic behind platform integrity</p>
          </div>
          <div className="flex flex-wrap justify-center gap-16">
            {[
              { img: dev1, name: 'Aditya Kumar Singh', role: 'Lead Designer & Frontend Architect' },
              { img: dev2, name: 'Sharad Polackal Sunil', role: 'ML Modeler & Backend Lead' },
              { img: dev3, name: 'Aditya Raj Gupta', role: 'ML Systems Designer' }
            ].map((member, idx) => (
              <div key={idx} className="flex flex-col items-center group">
                <div className="w-48 h-48 rounded-full overflow-hidden border-4 border-white/10 shadow-2xl group-hover:scale-105 group-hover:border-[#8083ff]/40 transition-all duration-300 relative">
                ...
                <h3 className="mt-8 text-xl font-black tracking-tight">{member.name}</h3>
                <p className={`${styles.textSecondary} text-[9px] uppercase tracking-[0.2em] mt-2 font-black max-w-[200px] text-center`}>{member.role}</p>
              </div>
            ))}
          </div>
        </section>

        <footer className="mt-64 w-full border-t border-white/5 pt-24 pb-12 flex flex-col md:flex-row justify-between items-center gap-12">
          <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-30">© 2026 VeriFace Platform</p>
          <div className="flex gap-12">
             <span className="text-[11px] font-bold opacity-40 hover:opacity-100 cursor-pointer transition-opacity">PRIVACY</span>
             <span className="text-[11px] font-bold opacity-40 hover:opacity-100 cursor-pointer transition-opacity">ACCESSIBILITY</span>
             <span className="text-[11px] font-bold opacity-40 hover:opacity-100 cursor-pointer transition-opacity">INTEGRITY</span>
          </div>
        </footer>
      </main>
    </div>
