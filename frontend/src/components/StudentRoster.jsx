...
export default function StudentRoster({ students, attendanceLogs = [], isDarkMode, onBack, title }) {
  ...
  const filteredStudents = studentAnalytics.filter(s => 
      ...
      <div className="max-w-[1400px] mx-auto space-y-12">
        <header className="flex flex-col xl:flex-row xl:items-end justify-between gap-12 border-b border-white/5 pb-12">
          <div>
            ...
            <h2 className="text-7xl font-black tracking-tighter leading-[0.85] font-headline">{title || 'Student Roster'}<br/><span className="opacity-40">{title ? 'Student List' : 'Roster'}</span></h2>
            <p className={`${styles.textSecondary} text-lg font-light mt-6 max-w-xl`}>The official list of students registered in the system. Real-time updates across all distributed nodes.</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 w-full xl:w-auto">
            <div className={`px-10 py-6 ${styles.surfaceLow} border border-white/5 rounded-[2rem] flex items-center gap-8 shadow-2xl`}>
               <div className="text-center">
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">Total Students</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-8">
          {filteredStudents.map(student => (
            <div key={student.id} className={`${styles.surfaceLow} p-10 rounded-[2.5rem] border border-white/5 hover:border-[#c0c1ff]/30 transition-all group shadow-2xl relative overflow-hidden flex flex-col justify-between`}>
              ...
              <div className="space-y-8 relative z-10 text-center">
                ...
                <div>
                  ...
                  <div className="flex items-center justify-center gap-2">
                     <span className="px-3 py-1 bg-[#c0c1ff]/10 text-[#c0c1ff] text-[9px] font-black uppercase tracking-[0.2em] rounded-full border border-[#c0c1ff]/20">Student</span>
                ...
                <div className="pt-8 border-t border-white/5 space-y-4">
                  <div className="flex justify-between items-end">
                     ...
                     <span className={`text-sm font-black tabular-nums ${parseFloat(student.rate) > 85 ? 'text-[#4edea3]' : 'text-[#ffb783]'}`}>
                        {student.rate}%
                     </span>
                  </div>
                  <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden shadow-inner">
                     <div 
                        className={`h-full transition-all duration-1000 ${parseFloat(student.rate) > 85 ? 'bg-[#4edea3]' : 'bg-[#ffb783]'}`} 
