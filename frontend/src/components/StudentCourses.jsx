    ...
    <div className="max-w-[1400px] mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/5 pb-8">
        <div>
          ...
          <p className={`${styles.textSecondary} text-lg font-light mt-4`}>The list of courses you are enrolled in.</p>
        </div>
          ...
          <div key={idx} className={`${styles.surfaceLow} p-10 rounded-[2.5rem] border border-white/5 hover:border-[#c0c1ff]/30 transition-all group shadow-2xl relative overflow-hidden`}>
            ...
            <div className="flex justify-between items-start mb-10">
              ...
              <div className="text-right">
                ...
                <p className={`text-[10px] font-black uppercase tracking-widest mt-2 ${parseFloat(course.attendance_rate) >= 75 ? 'text-[#4edea3]' : 'text-[#ffb783]'}`}>
            ...
            <div className="space-y-6">
               ...
               <div className="flex items-center gap-3">
                  ...
                  <p className={`text-[11px] ${styles.textSecondary} font-black uppercase tracking-widest`}>Classroom</p>
               </div>
            </div>
            
            <div className="mt-12 flex items-center justify-between border-t border-white/5 pt-8">
               <span className={`text-[10px] font-black uppercase tracking-[0.2em] px-4 py-1.5 rounded-full ${styles.surfaceHighest}`}>Active</span>
               <button className={`${styles.accentPrimary} text-[10px] font-black uppercase tracking-[0.2em] hover:underline underline-offset-8`}>Course Details</button>
            </div>
          </div>
        ))}
        {courses.length === 0 && (
          <div className="col-span-full py-32 text-center opacity-20 border-2 border-dashed border-white/5 rounded-[2.5rem]">
             <p className="text-sm font-black uppercase tracking-[0.4em] italic leading-relaxed">No courses found for your account.</p>
          </div>
        )}
      </div>
    </div>
