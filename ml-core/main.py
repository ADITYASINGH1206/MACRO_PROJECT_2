...
# Global executor to manage background network I/O gracefully
...
def main():
    ...
    try:
        while True:
            ...
            # Downscale native capture to 640x480 for massive YOLO speedup
                        ...
                        if encodings:
                            ...
                            if match:
                                ...
                                if s_id not in processed_students:
                                    print(f"[*] Queueing attendance for {match['name']}...")
                                    attendance_executor.submit(send_attendance_payload, s_id)
        ...
        print("[*] Flushing pending attendance requests...")
        attendance_executor.shutdown(wait=True)
        video_stream.stop()
